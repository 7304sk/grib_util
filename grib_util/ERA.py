...
def downloader(savedir:str, variables:list, years:list, months:list, area:list, h='single')->None:
    args = {
    ...
    if not str(h) in pressure_levels and h != 'single':
        raise ValueError(
            'Pressure height is not correctly specified. if you want to know the list of options, do "print(grib_util.ERA.pressure_levels)".')

    if h == 'single':
        dir_lev = 'single_level'
        data_target = 'reanalysis-era5-single-levels'
        for v in variables:
            if not v in single_level_var:
                raise ValueError(
                    f'"{v}" is not the correct option for "variables". if you want to know the list of options, do "print(grib_util.ERA.single_level_var)".')
    else:
        dir_lev = f'pressure_level/{h}'
        data_target = 'reanalysis-era5-pressure-levels'
        args['pressure_level'] = str(h)
        for v in variables:
            if not v in pressure_level_var:
                raise ValueError(
                    f'"{v}" is not the correct option for "variables". if you want to know the list of options, do "print(grib_util.ERA.pressure_level_var)".')
    basepath = os.path.join(savedir, dir_lev)

    c = cdsapi.Client()
    now_ym = dt.date.today()
    for v in variables:
        for y in years:
            savepath = f'{basepath}/{v}/{y}'
            ...
            for m in months:
                ...
                if the_ym >= now_ym:
                    print(f'  > ! data does not exist yet.')
                    continue
                
                args['variable'] = [v]
                args['year'] = [str(y).zfill(4)]
                args['month'] = [str(m).zfill(2)]
                c.retrieve(data_target, args, filename)

class reader:
    def __init__(self, ipt_dir, s_datetime, e_datetime, variable, h='single'):
        dir_lev = 'single_level/' if h == 'single' else f'pressure_level/{h}/'
        self.ipt_dir = os.path.join(ipt_dir, dir_lev)

        if not os.path.isdir(self.ipt_dir):
            raise ValueError(f'"{self.ipt_dir}" is not exists.' )

        self.s_datetime = dt.datetime(*s_datetime)
        self.e_datetime = dt.datetime(*e_datetime)
        self.variable = variable

        mdelta = (self.e_datetime.year - self.s_datetime.year)*12 + \
            (self.e_datetime.month - self.s_datetime.month)
        datas = dict()
        self.first_flag = True
        for m in range(mdelta+1):
            the_ym = self.s_datetime + relativedelta(months=m)
            f = f'{self.ipt_dir}/{variable}/{the_ym.strftime("%Y/%Y%m")}-{variable}.grib' if h == 'single' else f'{self.ipt_dir}/{variable}/{the_ym.strftime("%Y/%Y%m")}-{h}hPa-{variable}.grib'
            if os.path.exists(f):
                datas.update(self.openGrib(f))
            else:
                print(f'{os.path.basename(f)} is not downloaded.')
        self.data = datas

    def openGrib(self, filename:str):
